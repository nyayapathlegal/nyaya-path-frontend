              ...
              {navItems.map((nav, index) => {
                ...
                return (
                  <div key={nav.id} className="relative">
                    <button
                      className={`relative flex items-center space-x-1 px-5 py-2.5 rounded-xl text-sm font-semibold text-black transition-all duration-300 ${
            ...
            <div className="max-h-[calc(100vh-4rem)] overflow-y-auto px-4 py-6 space-y-3">
              {navItems.map((nav) => (
                <div key={nav.id} className="space-y-2">
                  <button
                    className={`w-full flex items-center justify-between px-5 py-3.5 rounded-xl font-semibold transition-all duration-200 ${
                      activeDropdown === nav.id
                        ? "bg-white/15 text-black shadow-lg"
                        ...
                        <div key={cat.title} className="space-y-1">
                          <button
                            className={`w-full flex items-center justify-between px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 ${
                              hoveredCategory === cat.title
                                ? "bg-white/15 text-black shadow-md"
                                : "text-black hover:bg-white/10"
                            }`}
                            onClick={() =>
                              setHoveredCategory(
